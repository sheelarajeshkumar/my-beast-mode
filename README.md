...
- `my-beast-mode-view` for a private local RTK token-optimization and memory dashboard
- `my-beast-mode-recall` for focused searches across prior Beast Mode sessions
...
- local cross-project usage memory, agent comparison, and filtered recall
- privacy-safe project aliases plus sanitized PNG and JSON sharing
- data-health visibility for RTK history, memory parsing, and discovery freshness
...
`/my-beast-mode-view` generates a self-contained local web page from RTK's history database and My Beast Mode memory. It shows raw versus optimized token estimates, session attribution, saved tokens, savings percentage, averages, active days, daily trend, detailed project metrics, command categories, missed-savings coverage, period-over-period signals, agent comparison, and data health. Filters cover 7, 30, 90, or all available days.

The dashboard uses Python and browser-native HTML, CSS, JavaScript, canvas, and SVG. It has no server, framework, package install, telemetry, or remote API. The RTK database is opened read-only. Raw command arguments, prompts, source code, and `rtk discover` unsupported-command examples are never included in the generated page.
...
- Python 3.10 or newer
- the sibling `my-beast-mode` skill, which owns the memory format and search helper
...
```bash
npx skills add sheelarajeshkumar/my-beast-mode --skill my-beast-mode my-beast-mode-view my-beast-mode-recall
```
...
cp -R skills/my-beast-mode-recall .agents/skills/
```
...
Create a privacy-safe dashboard and aggregate JSON export:
...
Aliases replace exact project paths before data reaches the HTML. The alias file is a JSON object:

```json
{
  "/Users/me/work/payments": "Payments API",
  "/Users/me/work/storefront": "Storefront"
}
```

Without an alias, `--privacy` assigns stable labels such as `Project 01` for that generated snapshot. The dashboard's **Share PNG** and **Share JSON** buttons export aggregate metrics with anonymized project labels; they omit paths, prompts, commands, and memory summaries.
...
| Project detail | Raw, optimized, saved, rate, average, execution time, commands, days, Beast runs, last activity, and top categories |
| How RTK optimized | Savings grouped by safe command family, such as `git`, `pytest`, or `docker` |
| Session attribution | RTK commands and savings correlated to each Beast Mode session by project and timestamp |
| Signals | Current-period changes versus the preceding equal period, plus incomplete or failed-session warnings |
| Missed-savings coverage | Supported `rtk discover` opportunities, estimated potential savings, and adoption coverage |
| Agent comparison | Runs, completion, checks, duration, and attributed savings grouped by host agent |
| Beast Mode memory | Run count, completion rate, duration, checks, and recent sanitized activity |
| Data health | RTK version, record window, retention context, memory parse failures, and discovery availability |

Click a project row to change the “How” chart from all projects to that project. Session attribution is a local correlation by project and start/end time, not distributed tracing. Missed-savings coverage is a generation-time `rtk discover` snapshot; disable it with `--no-discover` when command-history analysis is not wanted. RTK estimates token counts from text size, so these values describe output compression and are not an API invoice or exact model-token count.
...
SESSION_ID=$(python skills/my-beast-mode/scripts/memory.py start \
  --task-type review --agent codex --orchestrator host \
  --summary "Review authentication change")
...
```text
/my-beast-mode-recall authentication
```

Portable explicit invocation:

```text
Use $my-beast-mode-recall to find successful authentication work from the last month.
```

Direct CLI examples:

```bash
python skills/my-beast-mode/scripts/memory.py recall authentication \
  --project payments --outcome success --agent codex --limit 10

python skills/my-beast-mode/scripts/memory.py recall \
  --date-start 2026-08-01 --date-end 2026-08-31 --format json
```

Recall filters records before rendering them and defaults to a small result set. It returns sanitized session summaries and metrics, never transcripts or raw command history.
...
- `rtk discover` unsupported-command examples are discarded before dashboard generation because they can contain complete shell text.
- Shared PNG and JSON exports contain aggregate metrics and anonymized project labels only.
...
python skills/my-beast-mode-recall/scripts/recall.py dashboard --limit 1
```
...
skills-ref validate skills/my-beast-mode-recall
```
...
skills.sh reads valid skills from public GitHub repositories. All three skill directories contain the required `SKILL.md`, `name`, and `description` fields.
...
3. Verify the [my-beast-mode catalog page](https://skills.sh/sheelarajeshkumar/my-beast-mode/my-beast-mode), the [my-beast-mode-view catalog page](https://skills.sh/sheelarajeshkumar/my-beast-mode/my-beast-mode-view), the [my-beast-mode-recall catalog page](https://skills.sh/sheelarajeshkumar/my-beast-mode/my-beast-mode-recall), and their generated security audits.
...
> Private local web dashboard for RTK optimization and My Beast Mode memory, with session attribution, trends, missed-savings coverage, agent comparison, privacy-safe exports, and data health.

Recall catalog description:

> Search sanitized My Beast Mode session memory by text, project, task type, outcome, agent, and date without loading full transcripts.
...
### Missed-savings coverage is unavailable

The rest of the dashboard still works. Confirm `rtk discover --all --format json` succeeds, then regenerate the snapshot. Use `--no-discover` to intentionally omit this panel's live analysis.

### Privacy mode still shows a recognizable project name

Aliases are intentionally human-readable. Remove the matching entry from `aliases.json` to receive a generated `Project NN` label, or omit `--aliases` entirely. Privacy mode never embeds the original path.
...
If an agent cannot write there, set `MY_BEAST_MODE_MEMORY` to a writable private path. Reinstall or update all three skills so the recorder, dashboard, and recall command use the same schema.

### Recall skill cannot find the memory helper

Install all three skills together. `my-beast-mode-recall` deliberately reuses `my-beast-mode/scripts/memory.py` instead of maintaining a second memory implementation.
